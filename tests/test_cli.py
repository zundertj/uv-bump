...
def test_cli() -> None:
    with patch("uv_bump.cli.parse_args") as mock_parse_args:  # noqa: SIM117
        with patch("uv_bump.cli.bump_pyproject") as mock_bump:
...
def test_parse_args() -> None:
    ...
    assert result == argparse.Namespace(pyproject_toml="world", verbose=False, upgrade=True)


def test_parse_args_no_upgrade() -> None:
    result = parse_args(["--pyproject-toml", "world", "--no-upgrade"])

    assert result == argparse.Namespace(pyproject_toml="world", verbose=False, upgrade=False)
